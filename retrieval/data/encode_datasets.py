...
class EmDataset(Dataset):
    ...
    def __init__(self,
        ...
        if self.is_query_embed:
            ...
            save_path = f"index/{data_name}_id2doc.json"
            if data_path.endswith("tsv"):
                ...
                self.data = []
                save_path = "/checkpoint/xwhan/cc_head_lm200_psg100/" + save_path
                with open(data_path) as csvfile:
                    lines = [line.replace('\0','') for line in tqdm(csvfile.readlines())]
                    ...
                    for row in tqdm(reader):
                        ...
                        #     text += "."
                        # if title.lower().strip().endswith("wikipedia"):
                        #     continue
                        self.data.append({"id": id_, "text": text, "title": title})
                ...
                #     _["title"] = normalize(_["title"])
                    ...
                    self.data.append(_)
            elif "merged" in data_path:
                self.data = [json.loads(l) for l in tqdm(open(data_path).readlines())]
            else:
