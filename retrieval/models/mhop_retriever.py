...
class MhopRetriever(nn.Module):
    ...
    def __init__(self,
        ...
        self.bert_c = AutoModel.from_pretrained(args.model_name)
        self.shared_encoder = args.shared_encoder

        if not args.shared_encoder:
            self.bert_q = AutoModel.from_pretrained(args.model_name)

    def forward(self, batch):
        c1 = self.bert_c(batch['c1_input_ids'], batch['c1_mask'], batch.get('c1_type_ids', None))[0][:, 0, :]
        c2 = self.bert_c(batch['c2_input_ids'], batch['c2_mask'], batch.get('c2_type_ids', None))[0][:, 0, :]

        neg_1 = self.bert_c(batch['neg1_input_ids'], batch['neg1_mask'], batch.get('neg1_type_ids', None))[0][:, 0, :]
        neg_2 = self.bert_c(batch['neg2_input_ids'], batch['neg2_mask'], batch.get('neg2_type_ids', None))[0][:, 0, :]

        if self.shared_encoder:
            q = self.bert_c(batch['q_input_ids'], batch['q_mask'], batch.get('q_type_ids', None))[0][:, 0, :]
            q_sp1 = self.bert_c(batch['q_sp_input_ids'], batch['q_sp_mask'], batch.get('q_sp_type_ids', None))[0][:, 0, :]
        else:
            q = self.bert_q(batch['q_input_ids'], batch['q_mask'], batch.get('q_type_ids', None))[0][:, 0, :]
            q_sp1 = self.bert_q(batch['q_sp_input_ids'], batch['q_sp_mask'], batch.get('q_sp_type_ids', None))[0][:, 0, :]
    ...
    def encode_q(self, input_ids, q_mask, q_type_ids):
        if self.shared_encoder:
            q_vector = self.bert_c(input_ids, q_mask, q_type_ids)[0][:, 0, :]
        else:
            q_vector = self.bert_q(input_ids, q_mask, q_type_ids)[0][:, 0, :]
        return q_vector
