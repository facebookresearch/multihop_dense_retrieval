...
class QAModel(nn.Module):
    ...
    def __init__(self,
        ...
        if args.model_name == "spanbert":
            self.encoder = BertModel.from_pretrained("/private/home/xwhan/span-bert")
        else:
        ...
        # self.ans_type = nn.Linear(config.hidden_size, 3) # yes, no, span

        # self.qa_drop = nn.Dropout(args.qa_drop)
        # self.rank_drop = nn.Dropout(args.rank_drop)

        if self.sp_pred:
            self.sp = nn.Linear(config.hidden_size, 1)
            # self.sp_drop = nn.Dropout(args.sp_drop)
    ...
    def forward(self, batch):
        ...
        # sequence_output = self.qa_drop(sequence_output)
        # pooled_output = self.rank_drop(pooled_output)
        ...
        if self.training:
            ...
            # rank_loss = F.binary_cross_entropy_with_logits(rank_score, batch["ans_covered"].float(), reduction="sum")
